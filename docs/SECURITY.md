...
- Microsoft Graph and SignalWire credentials are Worker secrets, never browser variables or D1 settings.
- Microsoft Graph uses only delegated `offline_access`, `Mail.Read`, and `Mail.Send` scopes.
...
- External SMS, Microsoft Graph ingestion, and Microsoft Graph replies each have server-enforced kill switches.
